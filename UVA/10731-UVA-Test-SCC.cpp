...
const int MAX_N = 26; // assign
vector<int> adj[MAX_N];
int n, totalNodes; // CAREFUL overwriting in main
    ...
    while(cin >> n && n)
    {
        ...
        for (int i = 0; i < compNum; ++i)
        {
            ...
            for (int j = 0; j < sz(ans[i]); ++j)
                cout << (char)(ans[i][j] + 'A') << (j == sz(ans[i]) - 1 ? '\n' : ' ');
        }
    }
    return 0;
}